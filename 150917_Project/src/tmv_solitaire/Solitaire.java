package tmv_solitaire;

/** teleginmv addition
 * Задание 2e. На пустую стопку расклада может быть положена любая фигурная карта (король, дама, валет), а не только король.
 * Решение: Добавил константы queen и knave, методы isQueen() и isKnave() в классе Card, проверку в методе canTake() класса TablePile на то,
 * что одна из стопок tableau пуста и выбранная для перемещения туда карта либо король, либо дама, либо валет.
 * 
 * Задание 3. Пасьянс «thumb and pouch» похож на «косынку» за исключением того, что карта может быть положена на другую карту следующего
 * по старшинству ранга и любой масти за исключением ее собственной.
 * Решение: Заменил метод color() на colorAlt() в классе Card, который вместо цвета возвращает масть и изменил проверку в методе canTake()
 * класса TablePile на то, что масти не совпадают.
 * 
 * Задание 2a. Если пользователь щелкнул мышью по пустой стопке колоды, то промежуточная стопка перемещается (возможно, с перемешиванием)
 * назад в колоду. Таким образом пользователь может перебирать колоду неоднократно.
 * Решение: в коде присутствует
 * 
 * Задание 2b. Карты могут быть передвинуты из оснований назад в стопку расклада.
 * 
 * Задание 2c. Карты вытягиваются из колоды по три сразу и располагаются в промежуточной стопке в обратном порядке. Как и прежде, в игре
 * участвует самая верхняя карта в промежуточной стопке. Если в колоде остается меньше трех карт, то все оставшиеся карты перемещаются
 * в промежуточную стопку.
 * 
 * Задание 2d. То же, что вариант в, но в игре принимает участие любая из трех карт промежуточной стопки. (Это требует небольшого изменения во
 *  внешнем виде карточного стола, и больших исправлений в классе промежуточной стопки.)
 * 
 */
...
class Card 
{
  ...
  final static int queen   = 11;	//teleginmv addition
  final static int knave   = 10; 	//teleginmv addition
...
//true if card is queen, false otherwise
//teleginmv addition
...
//true if card is knave, false otherwise
//teleginmv addition
...
class TablePile extends CardPile 
{
  ...
  {
    if (empty())
      return aCard.isKing() || aCard.isQueen() || aCard.isKnave();	//teleginmv addition
    ...
    return (aCard.colorAlt() != topCard.colorAlt()) &&
           (aCard.rank() == topCard.rank() - 1);				//teleginmv addition method colorAlt()
  }
